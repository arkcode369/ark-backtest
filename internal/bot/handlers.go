...
func (b *Bot) handleMessage(msg *tgbotapi.Message) {
	...
	default:
		if text != "" && !strings.HasPrefix(text, "/") {
			// In strategy session check
			if b.stratSessions[chatID] {
				b.handleStrategyChat(chatID, text)
			}
		} else if cmd != "" {
			b.send(chatID, "❓ Unknown command. Use /help for the list of commands.")
		}
	}
}
...
Welcome! I help you backtest trading strategies on futures, CFDs, forex, metals, indices, and energy markets using free Yahoo Finance data — powered by AI analytics.

*Quick Start:*
• /symbols — browse all supported instruments
• /price XAUUSD — get latest price
• /backtest XAUUSD 1d ema\_cross — run a backtest
• /strategy — start AI-powered strategy creation
...
func (b *Bot) handleBacktest(chatID int64, args string) {
	...
	if len(result.Trades) > 0 {
		...
		b.sendMD(chatID, "_💡 Tip: Reply /analyze to get AI analysis of this backtest result, or /strategy to build a custom strategy._")
	}
}
...
func (b *Bot) handleGenMD(chatID int64, args string) {
	...
	doc.Caption = fmt.Sprintf("📄 Strategy: %s\n\nGenerated by AI Trading Backtest Bot", stratName)
	b.api.Send(doc)

	// Also send first 3000 chars as preview
	preview := md
	if len(preview) > 3000 {
		preview = preview[:3000] + "\n\n...(see full document above)"
	}
	b.send(chatID, preview)
}
